...
export const config: WebdriverIO.Config = {
    ...
    before: function (capabilities, specs) {
        browser.addCommand('waitAndClick', function(selector: WebdriverIO.Element) {
            try{    
                selector.waitForExist();
                selector.click();
            }catch(error) {
                throw new Error(`Could not click the element ${selector}`);
            }
        });

        browser.addCommand('verifyText', function(selector: WebdriverIO.Element, text: string){
            try{
                selector.waitForExist();
                expect(selector).toBeVisible();
                expect(selector).toHaveText(text);
            }catch(error){
                throw new Error(`Could not verify the text on element ${selector}`);
            }
        });
    },
    /**
